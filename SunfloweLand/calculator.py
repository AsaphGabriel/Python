...
def reduce_seeds_and_generate_report():
    ...
    for seed, initial_stock in seeds_per_blockbuck.items():
        ...
        while initial_stock > 0:
            if initial_stock < cropPlot:
                total_planted += initial_stock
                initial_stock = 0
            else:
                initial_stock -= cropPlot
                total_planted += cropPlot
            total_time += growth_time
            cycles += 1
        day = total_time.days
        if day not in daily_report:
            daily_report[day] = []
        daily_report[day].append(f"{seed} x{cycles} ({total_planted * buff:.2f} seeds)")
